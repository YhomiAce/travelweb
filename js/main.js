...
/*------------------------------------------------------------------------------*/
    ...
    $('#Send_team').click(e=>{
        ...
        if($('#team_form')[0].checkValidity()){
            e.preventDefault()
            $('#Send_team').val('Sending Message...')
            $.ajax({
                url:'inc/sendEmail.php',
                type:'post',
                data:$('#team_form').serialize()+'&action=Send_team_member',
                success:(res)=>{
                    console.log(res)
                    $('#Send_team').val('Send Message')
                    $('#team_form')[0].reset()
                    swal.fire({
                        title:"Your mail has been sent successfully",
                        icon:"success"
    ...
    $('.inquiry').click(e=>{
        ...
        if($('#immigration_form')[0].checkValidity()){
            e.preventDefault()
            $('.inquiry').val('Sending Message...')
            $.ajax({
                url:'inc/sendEmail.php',
                type:'post',
                data:$('#immigration_form').serialize()+'&action=make_inquiry',
                success:(res)=>{
                    console.log(res)
                    $('.inquiry').val('Send Message')
                    $('#immigration_form')[0].reset()
                    swal.fire({
                        title:"Your mail has been sent successfully",
                        icon:"success"
                    })
