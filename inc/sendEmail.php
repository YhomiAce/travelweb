<?php
	...
	// send team member message
	if(isset($_POST['action']) && $_POST['action'] == 'Send_team_member'){
		
		$name = htmlspecialchars($_POST['team_name']);
		$email = htmlspecialchars($_POST['team_email']);
		$data = htmlspecialchars($_POST['team_message']);
	...
	// Make Enquiry
	if(isset($_POST['action']) && $_POST['action'] == 'make_inquiry'){
		
		$name = htmlspecialchars($_POST['name2']);
		$email = htmlspecialchars($_POST['phone2']);
		$data = htmlspecialchars($_POST['message2']);
		$visa = htmlspecialchars($_POST['search_category']);
		...
		$message .= '<h1>Hello From Kreative!</h1>';
		$message .= '<div>';
		$message .= '<h2>Hello, I want to make an enquiry about your '.$visa.' program </h2>';
		$message .= '<h2>Name:  '.$name.' </h2>';
		$message .= '<h2>Visa Enquiry:  '.$visa.' </h2>';
		$message .= '<p>'.$data.' </p>';
		$message .= '</div>';
		$message .= "</body></html>";
		// if(mail($to,$subject,$message,$headers)){
